...
create_pr() {
  ...
  gh pr create --repo "${REPO}" --base "${BASE}" --head "${branch}" \
    --title "Test PR-${n}" --body "Trivial change for experiment"
}
...
BUMP_CONCLUSIONS=$(gh api "repos/${REPO}/actions/workflows/version-bump.yml/runs?branch=${BASE}&per_page=4" \
  --jq '.workflow_runs | map(.conclusion) | join(", ")')
BUMP_ALL_SUCCESS=true
if [ -z "$BUMP_CONCLUSIONS" ] || echo "$BUMP_CONCLUSIONS" | grep -qvE '^success(, success)*$'; then
