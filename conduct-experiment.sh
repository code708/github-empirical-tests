...
RUN_ID="merge-queue-wait-for-post-merge-workflow-to-finish-$(date +%Y-%m-%dT%H-%M)"
...
# Build SHA-to-PR mapping from merge commit SHAs
SHA_PR_MAP=""
MERGE_SHAS=""
for pr_url in "$PR1" "$PR2" "$PR3" "$PR4"; do
  pr_num="${pr_url##*/}"
  merge_sha=$(gh pr view "${pr_url}" --repo "${REPO}" --json mergeCommit --jq '.mergeCommit.oid' 2>/dev/null || echo "")
  if [ -n "$merge_sha" ]; then
    SHA_PR_MAP="${SHA_PR_MAP}${merge_sha}:${pr_num}"$'\n'
    MERGE_SHAS="${MERGE_SHAS}\"${merge_sha}\","
  fi
done

# Fetch version-bump runs scoped to this experiment's merge commits
BUMP_RUNS=$(gh api "repos/${REPO}/actions/workflows/version-bump.yml/runs?branch=${BASE}&per_page=20" \
  --jq "[.workflow_runs[] | select(.conclusion != \"skipped\") | select(.head_sha as \$sha | [${MERGE_SHAS%,}] | index(\$sha)) | {id, conclusion, html_url, display_title, head_sha}]")

sha_to_pr() {
  echo "$SHA_PR_MAP" | grep "^$1:" | cut -d: -f2 || echo "unknown"
}

BUMP_ALL_SUCCESS=true
BUMP_DETAILS=""
while IFS=$'\t' read -r title conclusion head_sha; do
  version=$(echo "$title" | grep -oE '[0-9]+$' || echo "?")
  status=${conclusion:-in_progress}
  pr_num=$(sha_to_pr "$head_sha")
  BUMP_DETAILS="${BUMP_DETAILS}- VERSION ${version} (PR #${pr_num}): ${status}\n"
  if [ "$status" != "success" ]; then
    BUMP_ALL_SUCCESS=false
  fi
  log "VERSION ${version} bump (PR #${pr_num}): ${status}"
done < <(echo "$BUMP_RUNS" | python3 -c "
import sys, json
for r in json.load(sys.stdin):
    print(f\"{r['display_title']}\t{r['conclusion'] or ''}\t{r['head_sha']}\")
")
if [ -z "$BUMP_DETAILS" ]; then
  BUMP_ALL_SUCCESS=false
  log "No version-bump runs found"
fi

if [ "$BUMP_ALL_SUCCESS" = false ]; then
  log "Fetching failed version-bump logs..."
  FAILED_IDS=$(echo "$BUMP_RUNS" | python3 -c "
import sys, json
...
INTERLEAVED=true
EXPECTED_LOG="merge-queue(wait-for-post-merge-workflow): Bump version to 4
...
ACTUAL_LOG=$(echo "$GIT_LOG" | grep -E '(Bump version to|Add test file for)' | head -8 || true)
...
gh api "repos/${REPO}/contents/runs/${RUN_ID}.md" \
  -X PUT \
  -f "message=test(exec): Record run results" \
